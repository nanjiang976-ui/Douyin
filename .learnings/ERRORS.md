...
- Recurrence-Count: 2
- Last-Seen: 2026-07-14T12:00:00+08:00
...
## [ERR-20260714-010] skill_metadata_description_length

**Logged**: 2026-07-14T12:20:00+08:00
**Priority**: low
**Status**: resolved
**Area**: docs

### Summary
更新 skill 的界面元数据时，首版中文短描述少于 25 字符，生成器拒绝写入。

### Suggested Fix
生成 `agents/openai.yaml` 前，先按 skill-creator 的 25—64 字符约束检查 `short_description`；失败后只调整界面文案并重跑生成器。

### Resolution
- **Resolved**: 2026-07-14T12:20:00+08:00
- **Notes**: 补足描述后，两套 skill 的 `agents/openai.yaml` 均已成功生成。

---

## [ERR-20260714-011] reviewed_markdown_validation_mode

**Logged**: 2026-07-14T12:30:00+08:00
**Priority**: low
**Status**: resolved
**Area**: tests

### Summary
对“如果说”的终审稿按纯文本模式校验，因文件带校订头信息而误报全部 65 篇含标题。

### Suggested Fix
对带 `*.reviewed.md` 的终审稿运行 `validate_benchmark_corpus.py` 时使用 `--reviewed-markdown`，只校验指定正文区段。

### Resolution
- **Resolved**: 2026-07-14T12:30:00+08:00
- **Notes**: 使用校订稿模式后，65 篇语料、5 个结构模型与开头指纹全部通过。
